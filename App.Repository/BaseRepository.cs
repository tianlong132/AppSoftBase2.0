...
namespace App.Repository
{
    public partial class BaseRepository<T> : IBaseRepository<T> where T : class, new()
    {
        ...
        public List<T> QueryByWherePage(int pageIndex, int pageSize, Expression<Func<T, bool>> whereExpression, string orderbyStr,ref int pageCount)
        {
            using (SqlSugarClient dbClient = SqlSugarInstance.GetInstance())
            {
                return dbClient.Queryable<T>().Where(whereExpression).OrderBy(orderbyStr).ToPageList(pageIndex, pageSize,ref pageCount);
            }
        }
        #endregion
        ...
        public List<T> QueryByWherePage(int pageIndex, int pageSize, ref int pageCount, Expression<Func<T, bool>> whereExpression, string orderbyStr, string whereString = "1=1", object whereObj = null)
        {
            using (SqlSugarClient dbClient = SqlSugarInstance.GetInstance())
            {
                return dbClient.Queryable<T>().Where(whereExpression).Where(whereString, whereObj).OrderBy(orderbyStr).ToPageList(pageIndex, pageSize,ref pageCount);
            }
        }
        #endregion
